...
async function runCaptureCycle(taskText) {
  ...
  try {
    ...
    */
    ...
    revealStep(
      "Sanitizing sensitive visual context locally…"
    );
    ...
    const steps = [
      {
        kind: "scan",
        title:
          `Observed ${scan.elements} UI elements, ` +
          `${scan.faces} face(s), ` +
          `${scan.textFields} text field(s)`,
        detail:
          scan.scanDetail ||
          "Local visual/DOM analysis"
      },
      {
        kind: "redact",
        title:
          "Sensitive data protected before network transmission",
        tags: (scan.redactions || []).map((r) => ({
          label: r,
          type: "pii"
        }))
      },
      {
        kind: "scan",
        title:
          "Sanitized context ready for server reasoning",
        detail:
          "Only safe structure, positions, and placeholder " +
          "tags are exposed to the reasoning service.",
        tags: [
          {
            label: "payload: sanitized",
            type: "safe"
          }
        ]
      }
    ...
    */
    ...
    revealStep(
      "Interpreting sanitized context…"
    );

    setProgress(66);

    const serverResult = await callServer(
      scan.sanitizedPayload,
      taskText
    );
  ...
  */
  ...
  return {
    ...
    actionPayload: {
      ...
      message:
        "Astra: sanitized context processed."
    },

    needsConfirmation: false,

    followUp:
      "Done — the sanitized page context was processed and the action was applied."
  };
}
...
async function runLocalScan() {
  /*
    ...
    Never put raw PII or unredacted pixels
    in sanitizedPayload.
  */
  ...
  return {
    ...
    scanDetail:
      "Local page scan • privacy filter active",

    followUp:
      "Done — the sanitized context was processed and the action was applied."
  };
}
