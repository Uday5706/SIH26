...
window.PIITextScanner = (function () {
  ...
  function scanTextNodes() {
    ...
    const boundingBoxes = [];
    const SKIP_TAGS = new Set(['SCRIPT', 'STYLE', 'NOSCRIPT', 'SVG', 'IFRAME', 'OBJECT']);
    ...
    const walker = document.createTreeWalker(
      ...
      NodeFilter.SHOW_TEXT,
      {
        acceptNode: function (node) {
          ...
          if (!content || content.length < 3) return NodeFilter.FILTER_SKIP;
    ...
    const maxNodes = 600; // Performance guard for huge pages

    while ((currentNode = walker.nextNode()) && nodesProcessed < maxNodes) {
      nodesProcessed++;
      const text = currentNode.textContent;
      
      // Fast heuristic check before running full regex suites
      const hasDigit = /\d/.test(text);
      const hasAt = text.includes('@');
      const hasKey = text.includes('key') || text.includes('token') || text.includes('pin') || text.includes('otp');
      if (!hasDigit && !hasAt && !hasKey) continue;
