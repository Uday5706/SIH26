...
const PrivacyGate = (function () {
  ...
  let ENABLE_VERBOSE_PAYLOAD_LOGGING = false;

  return {
    setVerboseLogging: function(enabled) {
      ENABLE_VERBOSE_PAYLOAD_LOGGING = enabled;
    },
    auditPayload: function(payload) {
      return deepSanitize(payload);
    },
    validatePayload: function(payload) {
      const { safePayload, modifications } = deepSanitize(payload);
      
      if (ENABLE_VERBOSE_PAYLOAD_LOGGING) {
        console.log("=== OUTBOUND PAYLOAD VERIFICATION LOG ===");
        console.log("EXACT SANITIZED OBJECT FOR NETWORK LAYER:");
